      ...
      <v-col class="pa-0 d-flex align-center justify-start"
        ><v-col-auto class="pa-0 w-auto custom_check mr-2"
          ><input
            ...
            @change="onChangeVal()"
        /></v-col-auto>
        {{ tableRow.description }}</v-col
      >
    </td>
    <td>{{ tableRow.price }}</td>
    <td>{{ tableRow.size }}</td>
    <td>{{ tableRow.eachBy }}</td>
    <td>
          ...
          >
            <v-row class="d-flex align-center">
              ...
              <v-col cols="12" class="d-flex">
                <fill-button @click="$emit('editModal')" class="small-font-cta fixed-width light-blue mr-2 pa-3"
...
import TicketView from '@/views/ticket-it/panel-view/adhoc-tickets/ticket-view/TicketView.vue'
